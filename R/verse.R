...
zfun_attach <- function() {
  ...
  msg(
    cli::rule(
      ...
      right = paste0("zfun ", package_version("zfun"))
    ),
    startup = TRUE
  )
...
#' Conflicts between the zfun and other packages
#'
...
#' @examples
#' zfun_conflicts()
zfun_conflicts <- function() {
  envs <- purrr::set_names(search())
  objs <- invert(lapply(envs, ls_env))

  conflicts <- purrr::keep(objs, ~ length(.x) > 1)

  pkg_names <- paste0("package:", zfun_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% pkg_names))

  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)

  structure(conflict_funs, class = "zfun_conflicts")
}

zfun_conflict_message <- function(x) {
  if (length(x) == 0) return("")

  header <- cli::rule(
    left = crayon::bold("Conflicts"),
    right = "zfun_conflicts()"
  )
...
#' @export
print.zfun_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(zfun_conflict_message(x))
}
...
#' Update zfun packages
#'
#' This will check to see if all zfun packages (and optionally, their
...
#' zfun_update()
#' }
zfun_update <- function(recursive = FALSE) {

  deps <- zfun_deps(recursive)
  behind <- dplyr::filter(deps, behind)

  if (nrow(behind) == 0) {
    cli::cat_line("All zfun packages up-to-date")
    return(invisible())
  }
...
#' List all zfun dependencies
#'
#' @param recursive If \code{TRUE}, will also list all dependencies of
#'   zfun packages.
#' @export
zfun_deps <- function(recursive = FALSE) {
  pkgs <- utils::available.packages()
  deps <- tools::package_dependencies("zfun", pkgs, recursive = recursive)
...
#' List all packages in the zfun
#'
#' @param include_self Include zfun in the list?
#' @export
#' @examples
#' zfun_packages()
zfun_packages <- function(include_self = TRUE) {
  raw <- utils::packageDescription("zfun")$Imports
...
.onAttach <- function(...) {
  ...
  if (!"package:conflicted" %in% search()) {
    x <- zfun_conflicts()
    msg(zfun_conflict_message(x), startup = TRUE)
  }
